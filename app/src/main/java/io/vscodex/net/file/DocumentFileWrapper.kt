...
data class DocumentFileWrapper(
    ...
    companion object {
        /**
         * Returns true if [DocumentFileWrapper] should be used for the given URI.
         *
         * Any `content://` URI must go through DocumentFile/ContentResolver because
         * there is no guaranteed mapping from a `content://` URI to a real filesystem
         * path (e.g. external storage tree URIs, SD-card URIs, Termux URIs, …).
         * The previous implementation only accepted `com.termux.documents`, which
         * caused "Unsupported Uri" crashes for standard Android storage tree URIs
